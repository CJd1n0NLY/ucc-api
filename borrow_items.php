...
if ($input) {
    ...
    if (empty($student_number) || empty($item_ids) || empty($branch_id)) {
        echo json_encode(["status" => "error", "message" => "Missing data or branch identity."]);
        exit();
    }
    ...
    try {
        ...
        foreach ($item_ids as $item_id) {
            $stmt->bind_param("sisssiiii", $student_number, $item_id, $borrow_date, $room, $teacher_name, $admin_id, $admin_id, $branch_id);
            $stmt->execute();
